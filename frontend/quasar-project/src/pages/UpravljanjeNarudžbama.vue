...
export default {
  ...
  setup() {
    ...
    const columns = [
      ...
      { name: 'sifraPlana', align: 'left', label: 'ID plana', field: 'sifraBiljke' },
    ...
    const novaNarudzba = ref({ nazivPlana: "", kolicina: 0, ID_korisnika: "", sifraPlana: "", total: 0 });
...
const datum = formatirajDatum("2024-06-12T17:19:23.000Z"); // Rezultat: "2024-06-12 17:19:23"
  ...
  try {
    ...
    // Clear the form
    novaNarudzba.value = { nazivPlana: "", kolicina: 0, ID_korisnika: "", sifraPlana: "" };
    prikaziDodajNarudzbu.value = false;

    // Optionally provide feedback
    alert("Uspješno ste dodali svoj plan prehrane!");
  } catch (error) {
  ...
  console.log('ID_Kosarice:', ID_Kosarice);
  try {
